...
public class SimpleChatWorkerThreadImpl extends AbstractWorkerThread {
  ...
  public SimpleChatWorkerThreadImpl(
      Connection con,
    ...
    try {
      Socket socket = new Socket("localhost", 50800);
      tcpConnect = new TcpConnector(socket);
      connectionType = 2;
    } catch (Exception e) {
  ...
  protected void chatMessageRequestAction(ChatPDU receivedPdu) {
    ...
    if (!clients.existsClient(receivedPdu.getUserName())) {
      ...
      if (pdu.getMessage().equals("shutdownAuditlog")) {
        ...
        if (client != null) {
          ChatPDU responsePdu =
              ChatPDU.createChatMessageResponsePdu(
                  receivedPdu.getUserName(),
                  0,
                  0,
                  0,
          ...
          try {
            ...
            log.debug(
                "Senden einer Chat-Message-Response-PDU an "
                    + client.getUserName()
                    + " nicht moeglich");
            ExceptionHandler.logExceptionAndTerminate(e);
          }
  ...
  private void sendLogoutResponse(String eventInitiatorClient) {
    ...
    if (client != null) {
      ChatPDU responsePdu =
          ChatPDU.createLogoutResponsePdu(
  ...
  protected void handleIncomingMessage() throws Exception {
    ...
    try {
      switch (receivedPdu.getPduType()) {
        ...
        default:
          log.debug(
              "Falsche PDU empfangen von Client: "
                  + receivedPdu.getUserName()
                  + ", PduType: "
