...
import edu.hm.dako.chat.Auditlog.TCPClient;
...
public class SimpleChatWorkerThreadImpl extends AbstractWorkerThread {
	...
	private TCPClient tcpClient = new TCPClient(new Socket("localhost", 14785));
	...
	@Override
	protected void logoutRequestAction(ChatPDU receivedPdu) throws IOException {
	...
	protected void chatMessageRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(receivedPdu.getUserName())) {
			...
			AuditlogPDU aPdu = AuditlogPDU.createChatMessageEventPdu(userName, receivedPdu);
