...
import edu.hm.dako.chat.UdpClient.UdpConnector;
...
public class SimpleChatWorkerThreadImpl extends AbstractWorkerThread {
  ...
  protected void loginRequestAction(ChatPDU receivedPdu) {
    udpConnect.sendMessage(new Date() + " " + receivedPdu.getUserName() + " logged in");
    ChatPDU pdu;
  ...
  protected void logoutRequestAction(ChatPDU receivedPdu) {
    udpConnect.sendMessage(new Date().toString() + " user " + receivedPdu.getUserName() + " tried to logout");
    ChatPDU pdu;
      ...
      for (String s : new Vector<String>(sendList)) {
        ...
        try {
          if ((client != null)
            ...
            client.getConnection().send(pdu);
            udpConnect.sendMessage("'" + pdu.getMessage() + "'" + " wurde als nachricht von " + pdu.getUserName() + " versendet");
